...
| --- | --- | --- |
| `gcPhase` | spike | **the field the GC finding rests on** — which top-level player-loop phase a collection *completed in*, not merely which frame it landed on. ~~Reads `TimeUpdate` on 22 of 23 in-raid collection frames across two maps.~~ **Corrected: 22 of 36.** The 23 was the population *where `gcPhase` was present*, and the field is omitted when no top-level counter moved — so the headline was computed over frames selected by the field's own success. Only emitted when `gcGen0 > 0`. |
...
| `gpu.render` | sample | `ProfilerRecorder` draw calls / SetPass / triangles. This build ships with the profiler enabled. ~~Draw calls track awake bots (r = +0.74) but **not** frame time (r = +0.06), so submission cost is not a lever here.~~ **Superseded 2026-07-28.** Against `render` rather than `frame` the correlation is **+0.717**, and **+0.683 with awake bots partialled out** — so draw calls are not bot count in disguise and submission *is* a lever. A small one: 0.47 µs per call, ~1.4 ms of a 6.6 ms `render`. Observational, n=38, and [Protocol B in TESTING.md](TESTING.md) is the intervention that would settle it. |
...
| `pos` / `at` | sample / spike | main player movement per window (`dist`, per-axis min/max, `end`) and the position of each spike. Exists because **position was an unmeasured confound under every cross-window comparison in this document** — no window on record travels less than 76 m. **Records position only, never look direction**, so the aim point cannot be verified from the log; `gpu.render.drawCalls.max ÷ .avg` is the stand-in (1.01 held, 1.63 median roaming). |
| `proc` | sample | working set, private commit, not-resident and `faults`, via a `psapi` P/Invoke. **Read the `err` key first.** `System.Diagnostics.Process.WorkingSet64` returns 0 under this Mono, and the previous implementation published those zeros for a day — so a failed read now emits `{"err": ...}` and never a number. Deltas are `null` on the first window of a run: "no baseline yet" is not "nothing moved". |
| `negResidualFrames` / `frameOverPeriodFrames`, and `*WorstMs` / `*SumMs` | sample | the two clock-disagreement signatures, counted over **every** frame rather than over spike lines. The mechanism moves time from line N to line N+1, so any magnitude filter is structurally blind to the follow-up line. `frameOverPeriodFrames` reading ~50% is **evidence the clocks agree**, not a defect rate — two clocks on the same span with symmetric noise give a coin flip. `SumMs ÷ Frames` is what separates jitter from mechanism. |
| `census.roots` | census | one entry per enumeration root, **including roots that resolved to null** — `"path": null` means "we looked for a weapon and there was not one", which is a finding, while omitting the entry looks fine. `path` is the hierarchy path, and it is there because one control bot returned **1,053 components from `ControllerGameObject` against 243 for its whole player subtree**: a weapon larger than the bot holding it is either a big prefab or a root that is not on the bot, and only the path tells them apart. |
