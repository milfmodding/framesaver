"""Package a Framesaver release zip: build, verify, gate, zip. Nothing clever.
...
already exist and this script RUNS them rather than replacing them:
  ...
  1. harness/check-release.py - Sophia's release gate. Public prose must be
     ...
     release-manifest.json) and that is correct: an undecided question is
     not an all-clear. This script inherits that refusal. When she writes
     the manifest and the gates pass, the same command starts producing a
     zip with no further ceremony - "push something out when we're ready"
     becomes one command at the moment of readiness, not a new checklist.

  2. The csproj's own stamp discipline (RefuseDirtyDeploy). A release zip
     built from a dirty tree ships a binary whose commit stamp names a
     commit whose source is not inside it. Deploy is opt-in in the build;
     packaging a zip is the same act with a wider audience, so the same
     refusal applies. Same for an unstamped binary: every telemetry log
     it ever writes reads commit:"" forever (see the csproj note - most
     of the existing corpus is already unrecoverable that way).

WHAT THE ZIP CONTAINS: BepInEx/plugins/Framesaver.dll at the path a user
...
def main():
    ...
    dll = os.path.join(REPO, "bin", "Release", "Framesaver.dll")
    ...
    version = "unknown"
    csproj = open(os.path.join(REPO, "Framesaver.csproj"),
    ...
    zpath = os.path.join(ARTIFACTS, "Framesaver-%s%s.zip" % (stamp, suffix))
    ...
    members = [("BepInEx/plugins/Framesaver.dll", dll)]
    ...
    for n in names:
        print("  %s" % n)
    print("  Framesaver.dll md5 %s" % dll_md5)
